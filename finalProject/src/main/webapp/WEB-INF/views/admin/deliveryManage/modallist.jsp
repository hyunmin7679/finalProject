					...
					<div>
									...
									<tbody>
										...
										<tr>
											...
											<td class="text-warning order-cancelAmount"
												data-cancelamount="0"><fmt:formatNumber value="${order.cancelAmount}"/></td>
										</tr>
										<tr>
						...
						<!-- <div class="text-end pe-4">
							<button class="btn btn-secondary p-2 m-0" type="button"
								id="routeVehicles">주문판매취소</button>
						</div> -->
						<div class="row delivery-update-area">
							<div class=" col pt-3">
							...
							<div
								class=" col pt-3 mb-3 pe-1 row justify-content-end ">
								<c:if test="${order.orderState > 1 && order.orderState < 5 }">
									<div class="col-4 product_status " style="width: 200px;">
										<select class="form-select text-capitalize delivery-select">
								...
								<c:if test="${order.orderState == 5}">
									<div class="col-7 product_status">
										<label><strong>배송완료 일자 : ${order.orderStateDate}</strong></label>
									</div>
								</c:if>
							</div>

							<c:if test="${order.orderState > 1 && order.orderState < 5 }">
								<div class="col-2 pt-3 mb-3 pe-1 text-center" style="width: 10%" id="DataTables_Table_0_length" >
									<button type="button" class="btn btn-secondary btn-delivery-order" data-orderNum="${order.orderNum}" data-orderState="${order.orderState }" >변경</button>
								</div>
							</c:if>
						</div>
								...
								<table class="table board-list order-detail-list p-0 m-0" style="width: 100%">
									<thead class="table-light m-0" style="text-align: center; font-size:15px; padding: 0">
										<tr>
											...
											<th width="100"style="padding: 0">쿠폰사용</th>
