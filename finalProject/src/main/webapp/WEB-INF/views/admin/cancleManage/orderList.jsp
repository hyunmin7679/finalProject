			...
			<div class="">
				<table class="table board-list order-detail-list p-0 m-0"
					...
					<thead class="table-light m-0"
						...
						<tr>
							<th width="110" style="padding: 0">취소신청일</th>
							...
							<th width="110" style="padding: 0"><div>상태</div>
							
							</th>
							<th width="100" style="padding: 0">
							
							
							<c:if test="${state =='ordercancle' }">
								취소처리
							</c:if>
							<c:if test="${state =='ordercanclecom' }">
								취소완료날짜
							</c:if>
							<c:if test="${state =='salecancle' }">
								취소완료날짜
							</c:if>
							</th>
					...
					<tbody style="text-align: center; width: auto; font-size: 12px;">
						...
						<c:forEach var="dto" items="${list}" varStatus="status">
							...
							<tr valign="middle" id="orderDetail-list${dto.orderDetailNum}"
								...
								<c:if test="${state=='salecancle'}">
								<fmt:formatNumber value="${dto.productCancelM}" />
								</c:if>
								</td>
								<td>${dto.payMethod }</td>
								<td>
